...
BASE=${TMPDIR:-/tmp}
RUN_DIR=$(mktemp -d "${BASE%/}/ds4-agent-self-improvement.XXXXXX")
HOME_DIR="$RUN_DIR/home"
REPO="$RUN_DIR/repo"
OUT="$RUN_DIR/output.txt"
TRACE="$RUN_DIR/trace.txt"
LEDGER="$RUN_DIR/ds4-self-improvement-ledger.md"
PROMPT_FILE="$RUN_DIR/prompt.md"
REPORT_DIR="$ROOT/tests/generated"
REPORT="$REPORT_DIR/ds4_agent_self_improvement_report.md"
REPORT_PROMPT="$REPORT_DIR/ds4_agent_self_improvement_prompt.md"
REPORT_OUTPUT="$REPORT_DIR/ds4_agent_self_improvement_output.txt"
REPORT_TRACE="$REPORT_DIR/ds4_agent_self_improvement_trace.txt"
REPORT_LEDGER="$REPORT_DIR/ds4_agent_self_improvement_ledger.md"
...
write_report() {
    ...
    {
        printf '# DS4 Agent Self Improvement Report\n\n'
...
cleanup() {
    ...
    if [ "${DS4_KEEP_SELF_IMPROVEMENT_TEST_DIR:-0}" != "1" ]; then
        rm -rf "$RUN_DIR"
    else
...
sed \
    ...
    "$ROOT/tests/ds4_agent_self_improvement_prompt.md" >"$PROMPT_FILE"

if ! HOME="$HOME_DIR" "$ROOT/ds4-agent" \
    --model "$ROOT/ds4flash.gguf" \
    --non-interactive \
    --chdir "$ROOT" \
    --ctx "${DS4_AGENT_SELF_IMPROVEMENT_CTX:-8192}" \
    --tokens "${DS4_AGENT_SELF_IMPROVEMENT_TOKENS:-4500}" \
    --temp 0 \
    --seed 11 \
    --nothink \
    --trace "$TRACE" \
...
then
    cat "$OUT" >&2
    echo "ds4-agent self-improvement run failed" >&2
    exit 1
fi

if [ ! -f "$LEDGER" ]; then
    cat "$OUT" >&2
    echo "missing generated self-improvement ledger: $LEDGER" >&2
    exit 1
fi

python3 "$REPO/test_toy_math.py" >/dev/null

grep -q "git_status_used=yes" "$LEDGER" || {
    cat "$LEDGER" >&2
    echo "ledger does not report git status usage" >&2
    exit 1
}

grep -q "git_diff_used=yes" "$LEDGER" || {
    cat "$LEDGER" >&2
    echo "ledger does not report git diff usage" >&2
    exit 1
}
...
grep -q "final=SELF_IMPROVEMENT_DONE" "$LEDGER" || {
    cat "$LEDGER" >&2
    echo "ledger is missing final marker" >&2
    exit 1
}

grep -q "SELF_IMPROVEMENT_DONE" "$OUT" || {
    cat "$OUT" >&2
    echo "DS4 did not finish self-improvement scenario" >&2
    exit 1
}

grep -Eq "git[[:space:]]+action=status" "$OUT" || {
    cat "$OUT" >&2
    echo "output does not prove git status tool was used" >&2
    exit 1
}

grep -Eq "git[[:space:]]+action=diff" "$OUT" || {
    cat "$OUT" >&2
    echo "output does not prove git diff tool was used" >&2
    exit 1
}
...
printf 'ds4 agent self-improvement e2e: ok\n'
