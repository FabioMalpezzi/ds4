...
static char *agent_tool_git(agent_worker *w, const agent_tool_call *call) {
    ...
    if (r.exit_code == 0 && !dry_run && agent_git_action_mutates(action)) {
        char detail[PATH_MAX + 320];
        if (message && message[0]) {
            snprintf(detail, sizeof(detail), "%s repo=%s path=%s message=\"%.*s\"",
                     action, opts.repo, path && path[0] ? path : (all ? "." : ""),
                     160, message);
        } else {
            snprintf(detail, sizeof(detail), "%s repo=%s path=%s",
                     action, opts.repo, path && path[0] ? path : (all ? "." : ""));
        }
        agent_context_note_side_effect(w, "git", detail);
    }
